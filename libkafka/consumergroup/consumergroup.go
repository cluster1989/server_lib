...
type Config struct {
	*sarama.Config
	...
	Offsets struct {
		Initial int64 // The initial offset method to use if the consumer has no previously stored offset.
...
type ConsumerGroup struct {
	...
	group    *libzookeeper.ConsumerGroup
	instance *libzookeeper.ConsumerGroupInstance
	...
	consumers libzookeeper.ConsumerGroupInstanceList

	offsetManager OffsetManager
}

// Connects to a consumer group, using Zookeeper for auto-discovery,// zkserver的地址，例如:localhost:2381,localhost:2382/test
func JoinConsumerGroup(name string, topics []string, config *Config) (cg *ConsumerGroup, err error) {
	...
	brokers, err := zk_help.Zookeeper.KafkaBrokerList()
	if err != nil {
		return nil, err
	}

	group := zk_help.Zookeeper.NewConsumerGroup(name)
	...
	if exists, err := cg.group.Exists(); err != nil {
		...
		logs.Info("Consumergroup `%s` is already  exists. start listen\n", cg.group.Name)
	}
	...
	if err := cg.instance.Register(topics); err != nil {
		...
		logs.Info("Consumer instance registered (%s).", cg.instance.ID)
	}
...
func (cg *ConsumerGroup) InstanceRegistered() (bool, error) {
	return cg.instance.IsRegistered()
}
...
func (cg *ConsumerGroup) topicListConsumer(topics []string) {
	for {
		...
		logs.Info("Currently registered consumers: %d\n", len(cg.consumers))
		...
		case <-consumerChanges:
			registered, err := cg.instance.IsRegistered()
			if err != nil {
...
func (cg *ConsumerGroup) topicConsumer(topic string, messages chan<- *sarama.ConsumerMessage, errors chan<- error, stopper <-chan struct{}) {
	...
	partitions, err := zk_help.Zookeeper.NewKafkaTopic(topic).Partitions()
	if err != nil {
...
func (cg *ConsumerGroup) partitionConsumer(topic string, partition int32, messages chan<- *sarama.ConsumerMessage, errors chan<- error, wg *sync.WaitGroup, stopper <-chan struct{}) {
	...
	for tries := 0; tries < maxRetries; tries++ {
		if err := cg.instance.ClaimPartition(topic, partition); err == nil {
			...
			if err == libzookeeper.ErrPartitionClaimedByOther {
