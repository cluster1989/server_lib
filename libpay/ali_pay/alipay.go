...
func signRSA2(keys []string, param url.Values, privateKey []byte) (s string, err error) {
	...
	sig, err := rsa_encrypt.SignPKCS1v15([]byte(src), privateKey, crypto.SHA256)
	if err != nil {
		return "", err
	}
...
func signRSA(keys []string, param url.Values, privateKey []byte) (s string, err error) {
	...
	var src = strings.Join(pList, "&")
	sig, err := rsa_encrypt.SignPKCS1v15([]byte(src), privateKey, crypto.SHA1)
	if err != nil {
		return "", err
	}
	s = base64.StdEncoding.EncodeToString(sig)
...
func verifySign(req *http.Request, key []byte) (ok bool, err error) {
	...
	if signType == K_SIGN_TYPE_RSA {
		err = rsa_encrypt.VerifyPKCS1v15([]byte(s), sign, key, crypto.SHA1)
	} else {
		err = rsa_encrypt.VerifyPKCS1v15([]byte(s), sign, key, crypto.SHA256)
	}
	if err != nil {
		return false, err
	}
...
func verifyResponseData(data []byte, signType, sign string, key []byte) (ok bool, err error) {
	...
	if signType == K_SIGN_TYPE_RSA {
		err = rsa_encrypt.VerifyPKCS1v15(data, signBytes, key, crypto.SHA1)
	} else {
		err = rsa_encrypt.VerifyPKCS1v15(data, signBytes, key, crypto.SHA256)
	}
	if err != nil {
		return false, err
	}
