...
func retrievePartitionLeaders(partitions libzookeeper.PartitionList) (partitionLeaders, error) {
...
func dividePartitionsBetweenConsumers(consumers libzookeeper.ConsumerGroupInstanceList, partitions partitionLeaders) map[string][]*libzookeeper.Partition {
	result := make(map[string][]*libzookeeper.Partition)
...
type partitionLeader struct {
	id        int32
	leader    int32
	partition *libzookeeper.Partition
}
